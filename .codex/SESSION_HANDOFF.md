...
- Cerrada localmente la version `0.19`: siete rarezas especiales con probabilidades 1-100, valores dinamicos persistidos, puntajes negativos, expiracion fugaz, falso sin agenda y gemelo con agenda doble. Los diez artes cuadrados estan en `assets/hisopos/`; comun/plateado/dorado tienen IDs validados y persistidos local/remotamente, mientras los siete especiales usan fallback comun hasta que el usuario aporte sus IDs. BotFather quedo sincronizado y verificado en `@galerazo_bot`. Sin reinicio ni deploy.
- El avatar generado para `Codex - Logs` esta en `assets/codex-logs.png`, pero Telegram rechazo el cambio: `@galerazo_bot` y `@testeoMensajePrivadoBot` ven el chat `-1004440313456` y ninguno tiene `can_change_info`. Reintentar solo despues de que el usuario otorgue a uno el permiso `Cambiar informacion del grupo`.
- Cerrada localmente la version `0.18`: el Recolector de Hisopos queda habilitado por defecto cuando el chat no tiene preferencia guardada, y un `enabled = 0` explicito se conserva. El repositorio publico tiene Secret Scanning y Push Protection habilitados, cero alertas abiertas, auditorias Gitleaks de historial y archivos versionados sin hallazgos, correo Git local `noreply` y ningun dato personal detectado en archivos versionados. No se reescribio historial, cambio el remoto ni desplego.
- Una captura de Hisopo edita la leyenda original, identifica ganador/rareza/puntos y elimina la botonera. Los puntajes negativos usan textos localizados de puntos perdidos y pueden dejar al usuario debajo de cero.
- El Recolector es configurable por grupo/supergrupo y habilitado por defecto sin sobrescribir desactivaciones explicitas; intensidades 1/5/10/15/20%, captura atomica, `/hisopos`, 18 idiomas, migracion completa y agenda SQLite para el dia siguiente.
- Para los siete IDs faltantes: enviar cada PNG a `@galerazo_bot`, responder la foto con `/debug` y copiar la ultima entrada de `message.photo[].file_id`; no usar `file_unique_id`.
...
- `Galerazo Bot - Logs.lnk` fue creado en `%USERPROFILE%\Documents\Codex\CODEX APPS`; sigue los logs remotos por IAP en PowerShell y se regenera con el setup/build.
...
- Version `0.19`: 262 pruebas nativas OK; cobertura 100% de sentencias y ramas; Python 3.14.7/lock, `pip check`, `compileall`, parse PowerShell/Bash, once PNG de 1254x1254, `git diff --check` y Gitleaks 8.30.1 sobre 172 archivos OK. Tres IDs base validados con ambos bots y persistidos remotamente sin reinicio; seis scopes BotFather verificados en `@galerazo_bot`. Sin Docker ni deploy.
- Version `0.18`: 259 pruebas nativas OK; cobertura 100% de sentencias y ramas; Python 3.14.7/lock, `pip check`, `compileall` y `git diff --check` OK. Auditoria Gitleaks 8.30.1 del historial y de 164 archivos versionados sin hallazgos; Secret Scanning y Push Protection habilitados con cero alertas; cero rutas absolutas de perfil y cero candidatos de correo personal versionados. Sin BotFather, Docker ni deploy.
...
No hay tarea de codigo activa. El usuario debe subir los siete PNG especiales a `@galerazo_bot`, responder cada foto con `/debug` y aportar la ultima entrada `message.photo[].file_id`; tambien debe otorgar `Cambiar informacion del grupo` a `@galerazo_bot` o `@testeoMensajePrivadoBot` para instalar `assets/codex-logs.png`. El proximo release debe incluir la version `0.19` y todas las correcciones acumuladas; usar Bot Control Center, vista Deploy, `Verificar` y `Publicar y deployar`, sin migrar datos locales.
