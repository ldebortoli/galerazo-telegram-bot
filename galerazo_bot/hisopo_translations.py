...
HISOPO_RACE_TRANSLATIONS: dict[str, dict[str, str]] = {
    "es": {
        ...
        "hisopos.race_button": "Pulsar · {current}/{target}",
        "hisopos.race_progress_caption": "¡Carrera por el {type_label}!\nLa marca más alta es {current}/{target} pulsaciones.",
    ...
    "en": {
        ...
        "hisopos.race_button": "Press · {current}/{target}",
        "hisopos.race_progress_caption": "Race for the {type_label}!\nThe highest score is {current}/{target} presses.",
    ...
    "ca": {
        ...
        "hisopos.race_button": "Prem · {current}/{target}",
        "hisopos.race_progress_caption": "Cursa pel {type_label}!\nLa marca més alta és {current}/{target} pulsacions.",
    ...
    "de": {
        ...
        "hisopos.race_button": "Drücken · {current}/{target}",
        "hisopos.race_progress_caption": "Rennen um das {type_label}!\nDer Höchststand ist {current}/{target} Klicks.",
    ...
    "eu": {
        ...
        "hisopos.race_button": "Sakatu · {current}/{target}",
        "hisopos.race_progress_caption": "{type_label} lortzeko lasterketa!\nMarkarik handiena {current}/{target} da.",
    ...
    "fr": {
        ...
        "hisopos.race_button": "Appuyer · {current}/{target}",
        "hisopos.race_progress_caption": "Course au {type_label} !\nLe meilleur score est de {current}/{target} pressions.",
    ...
    "gn": {
        ...
        "hisopos.race_button": "Ejopy · {current}/{target}",
        "hisopos.race_progress_caption": "Ñani {type_label} rehe!\nHetave jopyha ha'e {current}/{target}.",
    ...
    "it": {
        ...
        "hisopos.race_button": "Premi · {current}/{target}",
        "hisopos.race_progress_caption": "Corsa al {type_label}!\nIl record è {current}/{target} pressioni.",
    ...
    "ja": {
        ...
        "hisopos.race_button": "押す · {current}/{target}",
        "hisopos.race_progress_caption": "{type_label}争奪戦！\n現在の最高記録は{current}/{target}回です。",
    ...
    "la": {
        ...
        "hisopos.race_button": "Preme · {current}/{target}",
        "hisopos.race_progress_caption": "Certamen pro {type_label}!\nSumma nota est {current}/{target} pressiones.",
    ...
    "nl": {
        ...
        "hisopos.race_button": "Druk · {current}/{target}",
        "hisopos.race_progress_caption": "Race om de {type_label}!\nDe hoogste stand is {current}/{target} drukken.",
    ...
    "pt_BR": {
        ...
        "hisopos.race_button": "Tocar · {current}/{target}",
        "hisopos.race_progress_caption": "Corrida pelo {type_label}!\nA maior marca é {current}/{target} toques.",
    ...
    "pt_PT": {
        ...
        "hisopos.appeared_race": "Apareceu um {type_label}!\nA primeira pessoa a chegar a {target} toques vence.",
        "hisopos.race_button": "Premir · {current}/{target}",
        "hisopos.race_progress_caption": "Corrida pelo {type_label}!\nA melhor marca é {current}/{target} toques.",
    ...
    "quz": {
        ...
        "hisopos.race_button": "Ñit'iy · {current}/{target}",
        "hisopos.race_progress_caption": "{type_label} rayku kallpay!\nAswan hatun yupayqa {current}/{target} ñit'iy.",
    ...
    "ru": {
        ...
        "hisopos.race_button": "Нажать · {current}/{target}",
        "hisopos.race_progress_caption": "Гонка за {type_label}!\nЛучший результат: {current}/{target} нажатий.",
    ...
    "zh_Hans": {
        ...
        "hisopos.race_button": "按下 · {current}/{target}",
        "hisopos.race_progress_caption": "争夺{type_label}！\n当前最高为{current}/{target}次。",
    ...
    "zh_Hant": {
        ...
        "hisopos.race_button": "按下 · {current}/{target}",
        "hisopos.race_progress_caption": "爭奪{type_label}！\n目前最高為{current}/{target}次。",
        "hisopos.race_press_popup": "本次有效：{current}/{target}。",
...
HISOPO_RACE_RULE_UPDATES: dict[str, tuple[str, str, str]] = {
    "es": ("Común: 42,65 %", "Común: 34,65 %", "- Frenético: 4 %. La primera persona que llega a 20 pulsaciones gana +3 pt. Cada callback se cuenta una sola vez, hay 100 ms mínimos entre pulsaciones del mismo usuario y el progreso visible se actualiza como máximo cada 30 segundos, además del final.\n- Agujero negro: 4 % y carrera a 20. Si participa solo el ganador, suma +10 pt; con rivales, cada perdedor cede tantas unidades como pulsaciones hizo y el ganador recibe el menor valor entre 10 y el total cedido.\n- Vencido: el primer toque después del límite revela qué tipo era y no da puntos. Si no apareció como Misterioso, ese usuario colecciona un Vencido y la foto cambia a Vencido; si era Misterioso, revela la imagen real y no entrega Vencido."),
    "en": ("Common: 42.65%", "Common: 34.65%", "- Frenetic: 4%. The first person to reach 20 presses earns +3 pt. Each callback counts once, one user's counted presses must be at least 100 ms apart, and visible progress refreshes at most every 30 seconds plus the finish.\n- Black hole: 4% and a race to 20. A solo winner gets +10 pt; with rivals, every loser gives up one point per counted press and the winner receives the smaller of 10 or the total transferred.\n- Expired: the first late press reveals the original type and gives no points. A non-Mystery appearance gives that user one Expired collectible and changes to its image; a Mystery reveals the real image and gives no Expired collectible."),
    "es_ES": ("Común: 42,65 %", "Común: 34,65 %", "- Frenético: 4 %. La primera persona que llega a 20 pulsaciones gana +3 pt. Cada callback cuenta una vez, hay 100 ms mínimos entre pulsaciones del mismo usuario y el progreso visible se actualiza como máximo cada 30 segundos, además del final.\n- Agujero negro: 4 % y carrera a 20. Si solo participa quien gana, suma +10 pt; con rivales, cada perdedor cede tantos puntos como pulsaciones hizo y quien gana recibe el menor valor entre 10 y el total cedido.\n- Caducado: la primera pulsación tras el límite revela qué tipo era y no da puntos. Si no apareció como Misterioso, ese usuario colecciona un Caducado y cambia su imagen; si era Misterioso, revela la imagen real y no entrega Caducado."),
    "ca": ("Comú: 42,65 %", "Comú: 34,65 %", "- Frenètic: 4 %. La primera persona que arriba a 20 pulsacions guanya +3 pt. Cada callback compta una vegada, hi ha 100 ms mínims entre pulsacions d'un mateix usuari i el progrés visible s'actualitza com a màxim cada 30 segons, a més del final.\n- Forat negre: 4 % i cursa a 20. Si el guanyador participa sol, suma +10 pt; amb rivals, cada perdedor cedeix tants punts com pulsacions ha fet i el guanyador rep el menor entre 10 i el total cedit.\n- Caducat: la primera pulsació tardana revela el tipus i no dona punts. Si no apareixia com a Misteriós, l'usuari col·lecciona un Caducat i en mostra la imatge; si era Misteriós, mostra la imatge real i no dona Caducat."),
    "de": ("Gewöhnlich: 42,65 %", "Gewöhnlich: 34,65 %", "- Rasend: 4 %. Wer zuerst 20 Klicks erreicht, erhält +3 Pkt. Jeder Callback zählt einmal, gewertete Klicks derselben Person liegen mindestens 100 ms auseinander und der sichtbare Stand wird höchstens alle 30 Sekunden sowie am Ende aktualisiert.\n- Schwarzes Loch: 4 %, ebenfalls bis 20. Allein gibt es +10 Pkt.; mit Gegnern verliert jeder einen Punkt je eigenem Klick und der Sieger erhält höchstens 10 bzw. die kleinere übertragene Summe.\n- Abgelaufen: Der erste späte Klick enthüllt die Art, gibt aber keine Punkte. Außerhalb einer Mysteriösen Erscheinung sammelt die Person ein Abgelaufenes und dessen Bild erscheint; ein Mysteriöses enthüllt nur das echte Bild."),
    "eu": ("Arrunta: % 42,65", "Arrunta: % 34,65", "- Frenetikoa: % 4. 20 sakatze lortzen dituen lehenak +3 puntu irabazten ditu. Callback bakoitza behin zenbatzen da, erabiltzaile beraren sakatzeen artean gutxienez 100 ms daude eta ikusgai dagoen emaitza gehienez 30 segundoan behin eta amaieran eguneratzen da.\n- Zulo beltza: % 4 eta 20ra arteko lasterketa. Bakarrik badago, irabazleak +10 puntu; arerioekin, galtzaile bakoitzak bere sakatze adina puntu galtzen ditu eta irabazleak 10 eta guztizko transferentziaren arteko txikiena hartzen du.\n- Iraungia: berandu egindako lehen sakatzeak benetako mota erakusten du, punturik gabe. Ez bazen Misteriotsu agertu, erabiltzaileak Iraungia biltzen du; Misteriotsuak benetako irudia bakarrik erakusten du."),
    "fr": ("Commun : 42,65 %", "Commun : 34,65 %", "- Frénétique : 4 %. La première personne à 20 pressions gagne +3 pt. Chaque callback compte une fois, deux pressions comptées d'une même personne sont séparées d'au moins 100 ms et le progrès visible est actualisé au plus toutes les 30 secondes, plus la fin.\n- Trou noir : 4 %, course à 20. Seul, le gagnant reçoit +10 pt ; avec des adversaires, chacun perd autant de points que ses pressions et le gagnant reçoit le minimum entre 10 et le total transféré.\n- Expiré : la première pression tardive révèle le type sans point. Hors Mystérieux, cette personne collectionne un Expiré et son image apparaît ; un Mystérieux révèle l'image réelle sans donner d'Expiré."),
    "gn": ("Jepivegua: 42,65 %", "Jepivegua: 34,65 %", "- Sarambi: 4 %. Oguahẽva tenonde 20 jopyhápe ohupyty +3 kyta. Callback peteĩteĩ ojepapa peteĩ jevy, peteĩ puruhára jopyhakuéra apytépe oĩ 100 ms ha ojehecháva oñembopyahu 30 aravo'i rire térã opávo.\n- Kuára hũ: 4 % ha ñani 20 peve. Peteĩnte oñanírõ ohupyty +10; hetárõ, peteĩteĩ oguejy ojopy haguéicha ha oganáva ohupyty michĩvéva 10 térã kyta oñembohasáva apytépe.\n- Oñembyaíva: jopyha tenondegua ára rire ohechauka mba'eichagua kuri, kyta'ỹre. Ndaha'éirõ Ñemi, ojopyva ombyaty Oñembyaíva; Ñemi ohechauka ta'anga añetegua añoite."),
    "it": ("Comune: 42,65 %", "Comune: 34,65 %", "- Frenetico: 4 %. La prima persona a 20 pressioni guadagna +3 pt. Ogni callback conta una volta, le pressioni conteggiate dello stesso utente distano almeno 100 ms e il progresso visibile si aggiorna al massimo ogni 30 secondi, più il finale.\n- Buco nero: 4 %, corsa a 20. Da solo il vincitore prende +10 pt; con rivali, ognuno perde tanti punti quante pressioni e il vincitore riceve il minore fra 10 e il totale trasferito.\n- Scaduto: la prima pressione tardiva rivela il tipo senza punti. Fuori da un Misterioso, quella persona colleziona uno Scaduto e ne vede l'immagine; un Misterioso mostra l'immagine reale senza dare lo Scaduto."),
    "ja": ("通常：42.65%", "通常：34.65%", "- 熱狂：4%。最初に20回押した人が+3点を獲得します。各callbackは1回だけ数え、同じ人の有効な押下には100ms以上必要です。表示は最大30秒ごとと終了時に更新します。\n- ブラックホール：4%、20回の競争です。単独なら+10点。対戦者がいれば各敗者は自分の押下数ぶん失い、勝者は10点と移動総数の小さい方を得ます。\n- 期限切れ：期限後の最初の押下で正体を表示し、得点はありません。ミステリー表示でなければ押した人が期限切れを収集して画像も変わります。ミステリーなら本当の画像だけを表示します。"),
    "la": ("Commune: 42,65 %", "Commune: 34,65 %", "- Freneticum: 4 %. Primus ad 20 pressiones +3 puncta accipit. Quisque callback semel numeratur, eiusdem hominis pressiones numeratae saltem 100 ms distant, et progressus visibilis non saepius quam singulis 30 secundis atque in fine renovatur.\n- Foramen atrum: 4 %, certamen ad 20. Solus victor +10 accipit; cum aemulis, quisque victus tot puncta quot pressiones amittit et victor minorem inter 10 et summam translatam accipit.\n- Expletum: prima pressio sera genus aperit sine punctis. Nisi Mysteriosum visum erat, homo Expletum colligit et imago mutatur; Mysteriosum imaginem veram aperit nec Expletum dat."),
    "nl": ("Gewoon: 42,65%", "Gewoon: 34,65%", "- Razend: 4%. De eerste op 20 drukken krijgt +3 pt. Elke callback telt één keer, getelde drukken van dezelfde gebruiker liggen minstens 100 ms uiteen en de zichtbare stand wordt hoogstens elke 30 seconden en bij de finish bijgewerkt.\n- Zwart gat: 4%, race tot 20. Alleen krijgt de winnaar +10 pt; met rivalen verliest ieder punten gelijk aan diens drukken en krijgt de winnaar de kleinste waarde van 10 of het overgedragen totaal.\n- Verlopen: de eerste late druk onthult het type zonder punten. Buiten een Mysterie verzamelt die gebruiker een Verlopen en verschijnt die afbeelding; een Mysterie toont alleen de echte afbeelding."),
    "pt_BR": ("Comum: 42,65%", "Comum: 34,65%", "- Frenético: 4%. A primeira pessoa a 20 toques ganha +3 pt. Cada callback conta uma vez, os toques contados da mesma pessoa têm ao menos 100 ms de intervalo e o progresso visível atualiza no máximo a cada 30 segundos, além do final.\n- Buraco negro: 4%, corrida até 20. Sozinho, o vencedor ganha +10 pt; com rivais, cada perdedor cede pontos iguais aos próprios toques e o vencedor recebe o menor valor entre 10 e o total transferido.\n- Vencido: o primeiro toque atrasado revela o tipo sem dar pontos. Fora de um Misterioso, essa pessoa coleciona um Vencido e a imagem muda; um Misterioso só revela a imagem real."),
    "pt_PT": ("Comum: 42,65 %", "Comum: 34,65 %", "- Frenético: 4 %. A primeira pessoa a 20 toques ganha +3 pt. Cada callback conta uma vez, os toques contados da mesma pessoa têm pelo menos 100 ms de intervalo e o progresso visível atualiza no máximo a cada 30 segundos, além do final.\n- Buraco negro: 4 %, corrida até 20. Sozinho, o vencedor ganha +10 pt; com rivais, cada perdedor cede pontos iguais aos próprios toques e o vencedor recebe o menor valor entre 10 e o total transferido.\n- Expirado: o primeiro toque tardio revela o tipo sem dar pontos. Fora de um Misterioso, essa pessoa coleciona um Expirado e a imagem muda; um Misterioso só revela a imagem real."),
    "quz": ("Sapsi: 42,65 %", "Sapsi: 34,65 %", "- Phaway: 4 %. 20 ñit'iyta ñawpaq chayaqqa +3 puntuta hap'in. Sapa callback huk kutilla yupakun, kikin runapa ñit'iyninkunaqa 100 ms rakisqa, rikuchisqa yupaytaq 30 ch'illmi qhipamanta utaq tukukuypi musuqchakun.\n- Yana uchku: 4 %, 20 kama kallpay. Sapallan atipaq +10 puntuta hap'in; hukkunawanqa sapa mana atipaq ñit'isqan hina puntuta chinkachin, atipaqtaq 10 utaq llapan apachisqa ukhupi aswan huch'uyta hap'in.\n- Pacha tukusqa: qhipa ñawpaq ñit'iyqa imayna kasqanta rikuchin, mana puntuyuq. Mana Paka hina rikurirqan chayqa ñit'iq Pacha tukusqata huñun; Pakaqa chiqaq rikch'ayllata rikuchin."),
    "ru": ("Обычная: 42,65 %", "Обычная: 34,65 %", "- Неистовая: 4 %. Первый игрок с 20 нажатиями получает +3 очка. Каждый callback считается один раз, между засчитанными нажатиями одного игрока не менее 100 мс, видимый прогресс обновляется не чаще раза в 30 секунд и в финале.\n- Чёрная дыра: 4 %, гонка до 20. Единственный участник получает +10; с соперниками каждый проигравший теряет очки по числу своих нажатий, победитель получает минимум из 10 и общей передачи.\n- Просроченная: первое позднее нажатие раскрывает вид без очков. Если это не была Таинственная оболочка, игрок собирает Просроченную и видит её картинку; Таинственная показывает настоящий вид без Просроченной."),
    "zh_Hans": ("普通：42.65%", "普通：34.65%", "- 狂热：4%。最先按到20次的人获得+3分。每个callback只计一次，同一用户的有效按键至少间隔100毫秒；画面最多每30秒及结束时更新。\n- 黑洞：4%，按到20次。独自参赛得+10分；有对手时，每名败者按自己的有效次数失分，胜者获得10分与总转移分中的较小值。\n- 过期：超时后的首次按键会揭示原类型，但不得分。若外观不是神秘，按键者收藏一根过期棉签并更换图片；神秘只揭示真实图片，不给过期收藏。"),
    "zh_Hant": ("普通：42.65%", "普通：34.65%", "- 狂熱：4%。最先按到20次的人獲得+3分。每個callback只計一次，同一使用者的有效按鍵至少間隔100毫秒；畫面最多每30秒及結束時更新。\n- 黑洞：4%，按到20次。獨自參賽得+10分；有對手時，每名敗者按自己的有效次數失分，勝者獲得10分與總轉移分中的較小值。\n- 過期：逾時後的首次按鍵會揭示原類型，但不得分。若外觀不是神祕，按鍵者收藏一根過期棉花棒並更換圖片；神祕只揭示真實圖片，不給過期收藏。"),
}
