...
- [2026-07-20] Preparar deploy reproducible en Google Compute Engine: Docker multi-stage no root, Compose endurecido con persistencia, healthcheck SQLite, backup previo, rollback automatico, build/publicacion local, imagen manual desde GitHub Actions con WIF, bootstrap/deploy por IAP y guia completa sin secretos. Validacion local: 89 pruebas, compileall, runtime, pip check, PowerShell/Bash syntax y diff-check OK; Docker local no disponible y queda validado por Docker Quality al publicar el commit.
