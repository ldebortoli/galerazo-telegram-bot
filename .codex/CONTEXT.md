...
- El Recolector de Hisopos viene habilitado por defecto. Por cada mensaje valido tira contra 1/5/10/15/20% segun intensidad. La segunda tirada usa: comun 1-45 (+1), plateado 46-58 (+2), dorado 59-68 (+3), fugaz 69-75 (+5 y vence en un minuto), misterioso 76-82 (valor oculto ya persistido), putrefacto 83-87 (-2), radiactivo 88-91 (-3/-1/+2/+4/+6), falso 92-93 (0 y sin agenda), gemelo 94-95 (+4 y dos agendas) y diamante 96-100 (+10). Los puntajes pueden ser negativos. Las desactivaciones explicitas de cada chat se conservan y las capturas son atomicas.
...
- Las fotos del Recolector se reenvian con un `TELEGRAM_HISOPO_*_FILE_ID` por rareza. Comun, plateado y dorado estan configurados localmente y en el `bot.env` remoto persistente; los siete especiales esperan sus IDs. Si falta el ID sorteado, se usa el comun para no perder la aparicion. Los diez artes fuente versionados viven en `assets/hisopos/`.
...
- Al iniciar, el bot sincroniza los comandos sugeridos de BotFather por scope: privados reciben los comandos generales; grupos reciben comandos comunes excepto gastos y sus administradores reciben ademas los comandos `ADMIN`. Los comandos `DEV` y todos los de gastos no se sugieren; el menu global se limpia para no filtrarlos a canales. Toda sincronizacion manual se hace solo despues de verificar `getMe.username == "galerazo_bot"`; el `.env` local corresponde a `@testeoMensajePrivadoBot` y no es el destino de BotFather.
- `CURRENT_VERSION` es `0.19`; `CHANGELOG.md` contiene las notas por release y se actualiza con todo cambio funcional. El agente calcula la siguiente version salvo numero indicado por el usuario: capacidades importantes incrementan la menor y arreglos menores se agrupan como "Correcciones y mejoras". Si un comando cambia, tambien actualiza y sincroniza BotFather en esa ejecucion. SQLite anuncia cada version una vez al canal de novedades y `/version` la expone a todos.
