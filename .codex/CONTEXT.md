...
- La imagen mas reciente de Galerazo es `galerazobot:f9df2b1`, Linux/amd64, digest `sha256:03f5abdba1deac9e3787133a07f7592fa1ae060d5253b29036c2c01a0c6e5756`. Reemplaza para deploy a `db278a097b62`; `deploy/out/last-image.txt` conserva localmente la referencia mas reciente y esta ignorado por Git.
...
- El host `galerazo-prod` ejecuta `galerazobot:f9df2b1` mediante Compose con `network_mode: host` para reutilizar la salida IPv6 de la VM. El contenedor esta `running/healthy`, Telegram mantiene polling y envios HTTP 200, la politica de reinicio es `unless-stopped` y Docker esta activo/habilitado. `bot.env` permanece root/0600 sin OpenAI/Google Sheets y SQLite esta instalado como 10001:10001/0600, legible e integro.
