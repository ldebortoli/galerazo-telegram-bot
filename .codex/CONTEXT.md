...
- `scripts/deploy/`: ciclo GCP reproducible por etapas, build/publicacion local, bootstrap, deploy por IAP y rollback. `Initialize-GcpBot.ps1` asegura fundacion/identidad; `New-GceBotInstance.ps1` crea y valida red dual-stack, IAP y VM; `Invoke-GceBotLifecycle.ps1` orquesta Foundation/Infrastructure/Prepare/Publish/Deploy/Release/Rollback con confirmaciones y pausas de secretos/datos.
- `deploy/gce/`: scripts remotos idempotentes para instalar Docker/Cloud CLI, verificar servicio/permisos sin imprimir secretos, desplegar con healthcheck y restaurar la imagen anterior.
...
- El host `galerazo-prod` esta preparado con Docker Engine/Compose y Google Cloud CLI; Docker esta habilitado y los directorios de datos, backups y secretos tienen los owners/modos esperados. Todavia no contiene imagenes, contenedores ni SQLite del bot y `bot.env` conserva el placeholder.
