...
- `coleccionhisopos`: muestra la colección histórica del chat y el inventario cosmético global; al responder a otra persona, muestra los de ese usuario.
...
TELEGRAM_HISOPO_USED_FILE_ID=file-id-del-hisopo-usado
...
TELEGRAM_HISOPO_FRENETIC_FILE_ID=file-id-del-hisopo-frenetico
TELEGRAM_HISOPO_BLACK_HOLE_FILE_ID=file-id-del-hisopo-agujero-negro
TELEGRAM_HISOPO_EXPIRED_FILE_ID=file-id-del-hisopo-vencido
...
Cada mensaje original de un usuario humano que podría competir por La Galeraza genera una tirada de 1 a 100. Si la tirada entra en el porcentaje configurado, aparece un hisopo con foto y el botón `Capturar hisopo`; el Bomba usa un tablero de 16 botones y Frenético/Agujero negro usan un botón de carrera. Las ediciones y los mensajes de bots no generan tiradas.
...
El Misterioso contiene uno de los otros quince tipos sorteables, sin otro Misterioso ni un Vencido adentro. La selección interna conserva sus pesos relativos: común 31,88 %, usado 5,38 %, plateado 15,05 %, dorado 10,75 %, fugaz 7,53 %, putrefacto 5,38 %, radiactivo 4,30 %, bomba 4,30 %, frenético 4,30 %, agujero negro 4,30 %, falso 3,23 %, gemelo 2,15 %, diamante 1,08 %, gigante 0,27 % y milagroso 0,11 %. Su contenido y su valor permanecen ocultos hasta el primer clic. La envoltura Misteriosa siempre dura 20 minutos, incluso si contiene un Fugaz; en ese caso, los 5 puntos solo están disponibles durante el primer minuto. Desde el minuto 1 todavía puede reclamarse y revela el Fugaz, pero entrega 0 puntos y no agenda otra aparición. Si contiene un Gigante, Bomba, Frenético o Agujero negro, el primer clic revela la mecánica real y colecciona el Misterioso; el premio del tipo real se acredita a quien finalmente lo completa. Si vence antes del primer clic, revela el tipo y su foto pero no entrega ni Misterioso ni Vencido.
...
El Frenético dura 20 minutos. Cada callback nuevo incrementa en uno el contador persistente e individual de esa persona y la primera cuyo contador propio llega a 20 gana 3 puntos, lo agrega a su colección y programa el día siguiente. Un mismo `callback_query_id` nunca cuenta dos veces y se exige un mínimo de 100 ms entre pulsaciones válidas de la misma persona. El botón y el mensaje no publican contadores ni la mejor marca: cada pulsación válida devuelve a quien la hizo una notificación privada no bloqueante con su progreso. El mensaje solo se edita para revelar una carrera oculta por un Misterioso y para mostrar el desenlace.
...
El tope elimina el crecimiento exponencial, aunque los puntajes siguen sin tener un límite global. Partiendo de cero y suponiendo que la misma persona capturara siempre el Milagroso, harían falta `9.223.372.036.854.787` capturas para que la suma siguiente superara el entero SQLite firmado de 64 bits. Incluso a una captura por segundo serían más de 292 millones de años.

Para los tipos de captura directa, la primera callback procesada para ese chat reclama el premio dentro de una transacción inmediata de SQLite; las siguientes muestran una alerta de Telegram sin sumar. El Bomba y las carreras aplican la misma exclusión atómica a sus estados. Al capturar un Falso, Putrefacto, Usado o Misterioso, el mismo mensaje reemplaza la foto por la del tipo real, informa el resultado y elimina la botonera. A los 20 minutos la aparición queda esperando el primer toque tardío: ese usuario no gana ni pierde puntos, ve qué tipo se venció y colecciona un Vencido; la foto cambia al arte genérico de Vencido. La única excepción es una aparición Misteriosa: revela su tipo y foto reales, pero no entrega Vencido. Los toques tardíos posteriores se descartan. Tocar un Fugaz después de su minuto y antes de los 20 minutos solo revela que se perdió su premio. Si una rareza todavía no tiene todos los `file_id` que necesita para aparecer y revelarse, esa tirada usa el Hisopo común para no perder el evento.

La colección persiste por usuario y grupo sin temporadas ni reinicios. Guarda cantidades para 17 categorías: los 16 tipos sorteables y el Vencido. Cada Misterioso nuevo suma una unidad de Misterioso y otra del tipo que revela cuando ese tipo se completa; si ocultaba un Fugaz reclamado después de su minuto, solo suma el Misterioso. Los Misteriosos anteriores no se reconstruyen retroactivamente. Cuando se completa un Gigante, todos los participantes agregan el Gigante; si apareció oculto como Misterioso, solo quien lo reveló agrega además el Misterioso. En la colección se muestra simplemente `hisopo gigante`, sin el calificativo cooperativo. Un Bomba entra únicamente si se desactiva y un Vencido solo por ser la primera persona que toca tarde una aparición no Misteriosa. Los tipos todavía no descubiertos se muestran con `❓` y los descubiertos con `✅`. `/coleccionhisopos` muestra primero las categorías del chat y debajo los 21 cosméticos globales, incluido el Estelar histórico, con cantidades y ceros para los no adquiridos; la explicación detallada de Misterioso, Gigante, carreras y vencimientos vive únicamente en `/reglashisopo`. La colección y las pulsaciones pendientes se migran y combinan si Telegram convierte el grupo en supergrupo.
...
`/coleccionhisopos` en privado muestra solo los 21 cosméticos globales del usuario que lo ejecuta, incluidas cantidades cero, y un botón Web App para abrir sus álbumes. Responder a otro mensaje en privado no cambia el dueño consultado. En grupos conserva las 17 categorías naturales del chat y, debajo, los cosméticos; puede consultar a otra persona respondiéndole, pero el enlace sigue firmado para quien ejecutó el comando. La Mini App conserva el selector de grupos, vista conjunta, tienda, regalos y apoyo.

`/donar` crea facturas en Telegram Stars ligadas a la persona que ejecutó el comando. Permite aportes voluntarios de 25, 100 o 500 Stars, sumarse al Club del Hisopo por 100 Stars cada 30 días y conservar Cafecito como alternativa. El Club entrega un Estelar permanente al completar cada 90 días consecutivos pagos; el primero recién al día 90. No entrega puntos ni ventajas. Las donaciones no compran artículos; el Top colaboradores cuenta solamente donaciones confirmadas, descuenta reembolsos y muestra el nombre únicamente si la persona eligió hacerlo con `/donantes publico` o desde la Mini App. `/donantes anonimo` restaura el anonimato.
...
El Club se renueva cada 30 días y entrega un Estelar permanente por cada 90 días consecutivos pagos ya transcurridos. Pagar la tercera cuota al día 60 no adelanta el premio; el primero se acredita al día 90 y los siguientes al 180, 270, etc. Si vence y hay una interrupción, comienza una nueva racha; los bloques completos ganados se conservan. Cancelar la renovación no interrumpe la cobertura ya pagada. Un job local revisa los vencimientos cada minuto y al iniciar el bot, incluso con la Mini App pausada; también concilia al abrir el álbum o procesar pagos. No crea servicios ni consultas externas. El ledger SQLite y el inventario se actualizan en una transacción, recuperan entregas pendientes tras reinicios y evitan duplicados. Períodos superpuestos cuentan una sola vez. Las cuotas sin recompensa anteriores a 0.64 con vencimiento válido también cuentan; las cuotas históricas que ya dieron un Estelar quedan excluidas para no premiarlas dos veces. Reembolsar una cuota elimina su cobertura y recalcula solamente los premios trimestrales que corresponden; los regalos y premios históricos ajenos a ese reembolso se conservan. Los períodos sin fecha de vencimiento válida no se estiman como tiempo cumplido. Cancelar detiene renovaciones futuras; un reembolso confirmado ajusta el período de membresía o retira del destinatario el artículo asociado a una compra. Los cobros se procesan de forma idempotente por `telegram_payment_charge_id`, verifican moneda, importe, producto, comprador, destinatario y firma antes de acreditarse. `/paysupport` y `/terminos` permanecen disponibles aunque la Mini App no esté publicada.

Cada cobro nuevo confirmado por Telegram Stars se registra además en el canal de logging configurado: distingue donación, compra, regalo y cuota o renovación del Club; incluye comprador, importe, concepto, destinatario cuando corresponde, chat de origen y referencia del cobro. La idempotencia evita repetir el aviso si Telegram reentrega el mismo pago. Cafecito no forma parte de este registro porque ese servicio no envía sus cobros al bot mediante `successful_payment`.

La Mini App reúne tres pestañas: álbum, tienda y apoyo. Los botones nuevos de `/coleccionhisopos` en grupos usan un contexto compartible firmado `s1` con el titular que aparece en la respuesta, también al consultar respondiendo a otra persona. Quien visita ve sus cosméticos globales y sus capturas del grupo de origen con cantidades actuales; no necesita pertenecer al grupo, incluso si recibió el enlace reenviado. No recibe otros grupos, agregados naturales globales, datos del Club ni preferencias de aportes del titular. La vista indica «Álbum de…» y ofrece «Ver mi álbum». El titular conserva su vista completa. Los enlaces personales `a1`/`a2` anteriores continúan restringidos a su usuario. Desde `Mis álbumes` del privado/perfil se abre la cuenta autenticada con `Todos los grupos` y el selector de grupos propios. Los cosméticos mantienen la misma cantidad al cambiar de grupo. Telegram firma la identidad y el contexto en `initData`; las sesiones vencen en una hora. `view=mine` solo selecciona al visitante autenticado, nunca un titular arbitrario. Las facturas y preferencias siempre usan esa identidad autenticada. Este lote se prepara para octubre; no está desplegado todavía.
