...
- El host `galerazo-prod` esta preparado con Docker Engine/Compose y Google Cloud CLI; Docker esta habilitado y los directorios de datos, backups y secretos tienen los owners/modos esperados. `bot.env` real esta configurado como root 0600 sin OpenAI/Google Sheets y SQLite real esta instalado como 10001:10001/0600 con integridad verificada. `db278a097b62` llegó a desplegarse, pero quedó detenido tras 3081 reinicios causados por la red bridge sólo-IPv4. El Compose corregido con red de host está validado y pendiente de redeploy.
