...
No hay una implementacion activa. Versionado `0.1` implementado, validado, commiteado y pusheado: `CHANGELOG.md`, estado SQLite de anuncio unico, anuncio al iniciar una version nueva y `/version`. BotFather ya fue sincronizado directamente con los menus publicos. Las correcciones se acumulan localmente/Git y los releases de produccion quedan agrupados hasta que el usuario pida explicitamente publicar o desplegar.
...
- Rama `main`, tracking `origin/main`; ultimo commit propio `56e5cce` (`Add release versioning and announcements`) ya fue pusheado. `README.md` conserva una modificacion local ajena y no esta staged.
