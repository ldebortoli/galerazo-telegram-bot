...
- `scripts/deploy/`: ciclo GCP reproducible por etapas, build/publicacion local, bootstrap, configuracion secreta, deploy por IAP y rollback. `Initialize-GcpBot.ps1` asegura fundacion/identidad; `New-GceBotInstance.ps1` crea y valida red dual-stack, IAP y VM; `Set-GceBotSecrets.ps1` transfiere una lista permitida desde `.env` sin valores en argumentos; `Invoke-GceBotLifecycle.ps1` orquesta Foundation/Infrastructure/Prepare/Configure/Publish/Deploy/Release/Rollback con confirmaciones y pausas de datos.
- `deploy/gce/`: scripts remotos idempotentes para instalar Docker/Cloud CLI, instalar configuracion privada con rollback, verificar servicio/permisos sin imprimir secretos, desplegar con healthcheck y restaurar la imagen anterior.
