...
El Frenético dura 20 minutos. Cada callback nuevo incrementa en uno el contador persistente de esa persona y la primera que llega a 20 gana 3 puntos, lo agrega a su colección y programa el día siguiente. Un mismo `callback_query_id` nunca cuenta dos veces y se exige un mínimo de 100 ms entre pulsaciones válidas de la misma persona. Para cuidar los límites de Telegram, el mensaje visible se actualiza como máximo cada 30 segundos y siempre al terminar; cada callback se responde aunque no fuerce una edición.
