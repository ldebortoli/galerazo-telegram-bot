...
- `ApplicationBuilder` configura 30 segundos para los timeouts HTTP normales de Telegram; las respuestas paginadas y el aviso de ganador de La Galeraza no reintentan un `TimedOut` para evitar duplicados y lo registran como warning local. El punto de Galeraza ya persistido se conserva aunque falle ese aviso.
