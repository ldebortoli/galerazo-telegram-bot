...
- `ApplicationBuilder` configura 30 segundos para los timeouts HTTP normales de Telegram; las respuestas paginadas no reintentan un `TimedOut` para evitar duplicados. El timeout del aviso de ganador de La Galeraza se registra como error local con traceback y en el canal de logging, aclarando que el punto se conservo y que Telegram pudo haber entregado igualmente el aviso aunque no confirmo la respuesta; tampoco se reintenta.
