...
No hay tarea local activa. El siguiente paso del usuario es elegir/aprobar los tres artes y aportar sus `file_id` para la prueba local. El proximo release debe incluir la version `0.18` y todas las correcciones acumuladas; usar Bot Control Center, vista Deploy, `Verificar` y `Publicar y deployar`, sin migrar datos locales. Los demas pendientes requieren los datos o autorizaciones explicitados en BACKLOG.
